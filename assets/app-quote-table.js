function renderHoldingEditRow(card,rowNo,current={}){const id=holdingId(card),lotId=holdingInputState?.lotId||newHoldingLotId();return`
    <tr class="holding-row holding-edit-row ${card.userAdded?"user-holding-row":""}" data-holding-id="${esc(id)}" data-lot-id="${esc(lotId)}" title="${esc(card.key)} 보유 정보 입력">
      ...
      <td class="left holding-cell" colspan="${holdingsQuoteViewActive()?4:3}">
        <div class="holding-inline" data-holding-id="${esc(id)}" data-lot-id="${esc(lotId)}" data-after-lot-id="${esc(holdingInputState?.afterLotId||"")}" data-key="${esc(card.key)}">
          ...
          <span class="holding-inline-invested" data-holding-invested aria-live="polite">${holdingInvestedLabel(current.avg,current.qty)}</span>
        </div>
      </td>
    </tr>`}document.addEventListener("input",e=>{const t=e.target;if(!t||!(t.matches&&t.matches("[data-holding-avg],[data-holding-qty]")))return;const wrap=t.closest(".holding-inline");if(!wrap)return;const out=wrap.querySelector("[data-holding-invested]");!out||typeof holdingInvestedLabel!="function"||(out.textContent=holdingInvestedLabel(wrap.querySelector("[data-holding-avg]")&&wrap.querySelector("[data-holding-avg]").value,wrap.querySelector("[data-holding-qty]")&&wrap.querySelector("[data-holding-qty]").value))});function renderHoldingLotRow(card,rowNo,lot,index,total){const id=holdingId(card),calc=holdingDisplayCalc(card,lot);if(!calc)return"";const metric=holdingModeMetric(calc),returnClass=cls(metric.pct),valueClass=cls(metric.pnl),pnlText=metric.unavailable?"-":signedHoldingPlainAmountText(metric.pnl,calc.currency),pctText=metric.unavailable?"-":signedPctOne(metric.pct),titlePrefix=total>1?`보유 ${index+1} · `:"",dailyTitle=Number.isFinite(Number(calc.dayPnl))?` · 일일 손익 ${signedHoldingAmountText(calc.dayPnl,calc.currency)}`:"";return`
    <tr class="holding-row holding-lot-row${`${index===0?" holding-lot-first":""}${index===total-1?" holding-lot-last":""}`} ${card.userAdded?"user-holding-row":""}" data-holding-id="${esc(id)}" data-lot-id="${esc(lot.lotId)}" title="${esc(titlePrefix)}평가액 ${holdingSummaryMoneyText(calc.value,calc.currency)} · 원금 ${holdingSummaryMoneyText(calc.invested,calc.currency)} · 누적 손익 ${signedHoldingSummaryMoneyText(calc.pnl,calc.currency)}${dailyTitle} · 구매가격 ${holdingSummaryMoneyText(calc.avg,calc.currency)} · 수량 ${num(calc.qty)}">
      ...
      <td class="right holding-value-cell ${valueClass}">${esc(pnlText)}</td>${holdingsQuoteViewActive()?`
      <td class="right holding-eval-cell">${esc(holdingEvalAmountText(calc.value,calc.currency))}</td>`:""}
    ...
    <tr class="holding-empty-row">
      <td class="rownum">${rowNo}</td>
      <td class="left holding-empty-cell" colspan="${holdingsQuoteViewActive()?4:3}">
    ...
    </tr>`}function renderCashDisplayRow(c,rowNo,index,firstMovableRow,lastMovableRow){const id=String(c.cashId||"");if(cashInputState&&cashInputState.id===id)return renderCashEditRow(c,rowNo);const orderId=quoteRowOrderId(c),rowDragAttrs=` data-row-order-id="${esc(orderId)}"`,rowNumAttrs=` class="rownum quote-row-handle" data-row-order-id="${esc(orderId)}" title="행번호를 끌어서 순서 변경" aria-label="${esc(c.key)} 순서 변경"`,moveBtns=`<button class="row-move" data-action="move-default" data-dir="up" data-order-id="${esc(orderId)}" title="${esc(c.key)} 위로 이동" aria-label="위로 이동" ${index===firstMovableRow?"disabled":""}>▲</button><button class="row-move" data-action="move-default" data-dir="down" data-order-id="${esc(orderId)}" title="${esc(c.key)} 아래로 이동" aria-label="아래로 이동" ${index===lastMovableRow?"disabled":""}>▼</button>`,editBtn=`<button class="row-holding is-set" data-action="edit-cash-row" data-cash-id="${esc(id)}" title="${esc(c.key)} 현금 수정" aria-label="현금 수정">₩</button>`,removeBtn=`<button class="row-x" data-action="remove-cash-row" data-cash-id="${esc(id)}" title="${esc(c.key)} 삭제" aria-label="삭제">×</button>`,actions=`<span class="row-actions">${editBtn}${moveBtns}${removeBtn}</span>`,currency=normalizeCashPositionCurrency(c.cashCurrency,c.market),title=`${esc(c.market||"")} · ${esc(currency)} 현금 · ${fmtDt(c.asOf)}`;return`
    <tr class="cash-row"${rowDragAttrs} data-cash-id="${esc(id)}" data-outlook-badge="" data-outlook-tone="" title="${title}">
      ...
      <td class="right quote-price-cell">${cashAmountHtml(c.cashAmount,currency)}</td>${holdingsQuoteViewActive()?`
    ...
    <tr class="us-day-quote-notice-row">
      <td class="rownum">${rowNo}</td>
      <td class="left us-day-quote-notice-cell" colspan="${holdingsQuoteViewActive()?4:3}">미장 데이장은 자동 갱신 시세 미지원. 차트를 열었을 때만 현재가가 보입니다.</td>
    ...
    <tr>
      ...
      <th class="subhead">지표</th><th class="subhead">현재가</th><th class="subhead">평가액</th><th class="subhead">${changeHeaderLabel()}</th>
    </tr>`:`
    <tr>
      <th class="rownum"></th>
    ...
    </tr>`}let removeBtn="";if(c._hlProxy||MOOD_PROTECTED_KEYS.has(c.key))removeBtn="";else if(isUser){const marketAttr=`data-market="${esc(c.market||"")}"`,holdingBtn=canHoldCard(c)?`<button class="row-holding ${holdingFor(c)?"is-set":""}" data-action="edit-holding" data-holding-id="${esc(holdingId(c))}" data-key="${esc(c.key)}" data-price="${esc(c.price??"")}" title="${esc(c.key)} 구매가격/수량" aria-label="구매가격/수량">₩</button>`:"",moveBtns=`<button class="row-move" data-action="move-row" data-dir="up" data-code="${esc(c.code)}" ${marketAttr} title="${esc(c.key)} 위로 이동" aria-label="위로 이동" ${i===firstMovableRow?"disabled":""}>▲</button><button class="row-move" data-action="move-row" data-dir="down" data-code="${esc(c.code)}" ${marketAttr} title="${esc(c.key)} 아래로 이동" aria-label="아래로 이동" ${i===lastMovableRow?"disabled":""}>▼</button>`;removeBtn=`<span class="row-actions">${holdingBtn}${moveBtns}<button class="row-x" data-action="remove-row" data-code="${esc(c.code)}" ${marketAttr} title="${esc(c.key)} 삭제" aria-label="삭제">×</button></span>`}else{const orderId=cardOrderId(c),holdingBtn=canHoldCard(c)?`<button class="row-holding ${holdingFor(c)?"is-set":""}" data-action="edit-holding" data-holding-id="${esc(holdingId(c))}" data-key="${esc(c.key)}" data-price="${esc(c.price??"")}" title="${esc(c.key)} 구매가격/수량" aria-label="구매가격/수량">₩</button>`:"",moveBtns=`<button class="row-move" data-action="move-default" data-dir="up" data-order-id="${esc(orderId)}" title="${esc(c.key)} 위로 이동" aria-label="위로 이동" ${i===firstMovableRow?"disabled":""}>▲</button><button class="row-move" data-action="move-default" data-dir="down" data-order-id="${esc(orderId)}" title="${esc(c.key)} 아래로 이동" aria-label="아래로 이동" ${i===lastMovableRow?"disabled":""}>▼</button>`;removeBtn=`<span class="row-actions">${holdingBtn}${moveBtns}<button class="row-x" data-action="hide-default" data-key="${esc(c.key)}" title="${esc(c.key)} 숨기기" aria-label="숨기기">×</button></span>`}const live=c._hlProxy?"":liveBadgeHtml(c,session),trExtraCls=c._hlProxy?hlProxyRowClass(c):MOOD_PROTECTED_KEYS.has(c.key)?" mood-row":"",rowOrderId=quoteRowOrderId(c),rowDraggable=!c._hlProxy,rowDragAttrs=rowDraggable?` data-row-order-id="${esc(rowOrderId)}"`:"",rowNumAttrs=rowDraggable?` class="rownum quote-row-handle" data-row-order-id="${esc(rowOrderId)}" title="행번호를 끌어서 순서 변경" aria-label="${esc(c.key)} 순서 변경"`:' class="rownum"',{priceCell,changeCell,changeClass,previewChangeValue,changeTitle}=cardRenderedCells(c),quoteId=c._hlProxy?hlProxyQuoteId(c):quoteTokenForCard(c),tvSymbol=c._hlProxy?"":tradingViewSymbolForCard(c),tvTipPreferred=tradingViewTipPreferred(c,tvSymbol)?"1":"",tvButton=tvSymbol?`<button class="tv-chart-button" data-action="open-tv-chart" data-token="${esc(quoteId)}" data-tv-symbol="${esc(tvSymbol)}" data-label="${esc(c.key)}" title="${esc(c.key)} TradingView 차트" aria-label="${esc(c.key)} TradingView 차트">차트</button>`:"",holdingRows=renderHoldingRows(c,rowNo),holdingGroupCls=holdingRows.html?" quote-with-holding":"",rowTitle=c._hlProxy?"":`${c.market||""} · ${c.source||"-"} · ${fmtDt(c.asOf)}`,rowTitleAttr=rowTitle?` title="${esc(rowTitle)}"`:"",hlDataAttrs=c._hlProxy?` data-hl-proxy-id="${esc(c._hlId||"")}"`:"",labelExtra="",rawPriceValue=c.price===null||c.price===void 0||c.price===""?null:Number(c.price),priceValueAttr=Number.isFinite(rawPriceValue)?` data-quote-price-value="${esc(rawPriceValue)}"`:"";let rowHtml=`
    ...
    <tr class="summary-sheet-note-blank-row" aria-hidden="true">
      ...
      <td class="left"></td>
      <td class="right"></td>${holdingsView?`
      <td class="right"></td>`:""}
      <td class="right"></td>
    </tr>
    <tr class="summary-sheet-note-row" data-xk-area="summary" data-xk-position="summary-bottom" data-xk-id="sponsor-open" data-xk-label="알림" data-xk-variant-index="0" data-xk-variant-text="이곳에 한줄 광고를 넣어주실 광고주를 모십니다.">
      ...
      <td class="left summary-sheet-note-cell" colspan="${dataCols}">
